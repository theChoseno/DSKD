...
def eval(args):
    ...
    print(np.trapz([np.mean(num_correct_model)] + list(np.mean(num_correct_pertub, axis=1)), [0] + perturbation_steps))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch-size', type=int,
                        default=16,
                        help='')
    parser.add_argument('--neg', type=bool, default=False, help='')
    ...
    exp_name = args.type + '_' + args.model
#     pdb.set_trace()
    ...
    if args.vis_class == 'index':
        vis_method_dir = os.path.join(PATH,'visualizations/{}/{}_{}'.format(args.type + '_' + args.model,
                                                          args.vis_class,
                                                          args.class_id))
    else:
        ablation_fold = 'ablation' if args.is_ablation else 'not_ablation'
        vis_method_dir = os.path.join(PATH,'visualizations/{}/{}/{}'.format(args.type + '_' + args.model,
                                                       args.vis_class, ablation_fold))
