...
from bert_of_theseus.modeling_bert_of_theseus_svd import BertForSequenceClassificationSVD
from bert_of_theseus.replacement_scheduler import ConstantReplacementScheduler, LinearReplacementScheduler
# from bert_of_theseus.modeling_bert_of_theseus_svd import input3072_size
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassificationSVD, BertTokenizer),
}
...
def calculate_weight(step, all_steps):
    return step / all_steps

def train(args, train_dataset, model, tokenizer, if_svd, log_dir):
    """ Train the model """
    global replacing_rate_scheduler
    basic_choose_name = {}
    gradient_mask = dict()
    gradient_name_mask = dict()
    for name, params in model.bert.encoder.layer.named_parameters():
        if _determine_type(name) in 'FFN_1,FFN_2,attention' and name not in basic_choose_name:
            gradient_mask[params] = params.new_ones(params.size())
            gradient_name_mask[name] = 0
    choose_name = set()
    for target_name in ['intermediate.dense.weight', 'output.dense.weight', 'attention.self.query.weight',
                        'attention.self.key.weight', 'attention.self.value.weight', 'attention.output.dense.weight']:
        target_group = {k: v for k, v in gradient_name_mask.items() if
                        (target_name in k) and ("bias" not in k) and (target_name not in basic_choose_name)}
    ...
    gradient_mask = dict()
    gradient_name_mask = dict()
    for name, params in model.bert.encoder.scc_layer.named_parameters():
        if _determine_type(name) in 'FFN_1,FFN_2,attention' and name not in basic_choose_name:
            gradient_mask[params] = params.new_ones(params.size())
            gradient_name_mask[name] = 0
    choose_name = set()
    ...
    print('###################')
    teacher_svd_tensor = model.bert.encoder.teacher_layer_with_svd(choose_name, rank_indeces, k=args.rank_size)
    print('###################')
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs, scc_layer_index = model(**inputs)
            # loss_layer = model.bert.encoder.loss_layer_fn(choose_name)
            loss = outputs[0]
            ...
            if if_svd:
                svd_loss, student_svd_tensor = model.bert.encoder.compute_svd_loss(choose_name, teacher_svd_tensor,
                                                                                   rank_indeces, J_scores, k=args.rank_size)
                # print(svd_loss)
                loss = loss + args.lambda_size * svd_loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if if_svd and (global_step+1) % args.svd_steps == 0:
                    alpha_t = calculate_weight(global_step, t_total)
                    d_svd = DynamicRankDistiller(model.bert.encoder.scc_layer, choose_name, scc_layer_index, alpha_t,
                                                 k=args.rank_size)
                    rank_indeces, J_scores= d_svd.compute_J_scores(student_svd_tensor)
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results, result_test = evaluate(args, model, tokenizer)
                        ...
                        for key, value in result_test.items():
                            ...
                            logs_test[eval_key] = float(value)

                    if args.task_name == 'sts-b':
                        if logs['eval_corr'] > f1_mark:
                            f1_mark = logs['eval_corr']
                            f1_test = logs_test['test_corr']
                            log_dev = results
                            log_test = result_test
                            best_model_state = copy.deepcopy(model.state_dict())
                    elif args.task_name == 'rte' or args.task_name == 'sst-2' or args.task_name == 'qnli' or args.task_name == 'mnli':
                        if logs['eval_acc'] > f1_mark:
                            f1_mark = logs['eval_acc']
                            f1_test = logs_test['test_acc']
                            log_dev = results
                            log_test = result_test
                            best_model_state = copy.deepcopy(model.state_dict())
                    elif args.task_name == 'cola':
                        if logs['eval_mcc'] > f1_mark:
                            f1_mark = logs['eval_mcc']
                            f1_test = logs_test['test_mcc']
                            log_dev = results
                            log_test = result_test
                            best_model_state = copy.deepcopy(model.state_dict())
                    else:
                        if logs['eval_f1'] > f1_mark:
                            f1_mark = logs['eval_f1']
                            f1_test = logs_test['test_f1']
                            log_dev = results
                            log_test = result_test
                            best_model_state = copy.deepcopy(model.state_dict())
                    print("######################best##############################")
                    ...
                    file_path = f"C:/lyk/python/KD_BERT_SVD/{args.task_name}_{args.learning_rate}_{args.if_svd}_{args.svd_steps}_final_new_{args.num_train_epochs}_{args.rank_size}_{args.alpha_size}_{args.beta_size}+_map.txt"
    ...
    return global_step, tr_loss / global_step, best_model_state
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("  model size = %d",
                    sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad))
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = None
        out_label_ids = None
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if test:
            ...
            if task in ['mnli', 'mnli-mm'] and args.model_type in ['roberta']:
                # HACK(label indices are swapped in RoBERTa pretrained model)
                label_list[1], label_list[2] = label_list[2], label_list[1]
            if task != 'sts-b' and task != 'sst-2' and task != 'mnli-mm':
            ...
            elif task == 'sts-b' or task == 'rte' or task == 'sst-2' or task == 'qqp' or task == 'cola' or task == 'qnli' or task in [
                'mnli', 'mnli-mm']:
...
def main():
    ...
    parser.add_argument("--if_svd", default=True, type=bool,
                        help="if use DSKD")
    parser.add_argument("--svd_steps", default=100, type=int,
                        help="The SVD steps")
    parser.add_argument("--lambda_size", default=0.1, type=float,
                        help="lambda")
    parser.add_argument("--rank_size", default=32, type=int,
                        help="svd rank")
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_model = train(args, train_dataset, model, tokenizer, args.if_svd, log_dir)
